    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full overflow-x-auto">
        ...
        {loan.status === "APPROVED" && (
          <>
            <h1 className="text-2xl font-bold py-3 mt-5">Scheduled Repayments</h1>

            <Link href={`/manage-loans/pay-loan?loanId=${loan._id}`}>Pay Loan</Link>
                ...
                {loan?.scheduledRepayments?.map((repayment, index) => (
                  <tr key={index}>
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white">{repayment.amount}</p>
                    </td>
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p
                        className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${
